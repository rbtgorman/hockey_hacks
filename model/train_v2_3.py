"""
v2.3 xG model: v1 architecture + expanding-window shooter prior.

Identical to v2.2 except for the prior source. v2.2 used skater_priors_train,
which was a single per-(player, strength) value pooling 2022-23 + 2023-24 data
applied to all shots regardless of date. v2.3 uses skater_priors_expanding,
which gives each shot the prior available *at that game's date* using a
trailing 2-year window.

Compare against v2.2 directly — same hyperparams, same features, same query
...
LOAD_QUERY = """
    ...
    LEFT JOIN skater_priors_expanding sp
        ON sp.player_id = s.shooter_id
       AND sp.game_date = g.game_date
       ...
       END
    LEFT JOIN (
        -- Per-bucket league-mean fallback (used when a player has no prior row,
        -- e.g. their first-ever shot in a bucket)
        SELECT strength_state, AVG(prior_mean) AS league_mean
