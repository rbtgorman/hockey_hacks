...
from .config import SHIFTS_URL, LANDING_URL, RAW_DATA_DIR
...
def ingest_rosters_for_game(game_id: int, season: int) -> tuple[bool, int, str | None]:
    """Fetch + parse + insert roster for one game via /landing endpoint."""
    cache = RAW_DATA_DIR / f"landing/{season}/{game_id}.json"
    try:
        payload = fetch_json(LANDING_URL.format(game_id=game_id), cache_path=cache)
    except FetchError as e:
        return (False, 0, f"fetch: {e}")
    ...
    if not result.rosters:
        return (False, 0, f"no rosters parsed ({len(result.warnings)} warnings)")
